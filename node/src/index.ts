...
const checkAuth = async (req: express.Request) => {
    ...
    if (tokenRes.length && getTimestamp() - tokenRes[0].createdAt < 60 * 60 * 24) {
        return (await asyncQuery(`SELECT * FROM users WHERE passwordHash = '${tokenRes[0].fromUser}';`)).rows[0];
    }
}
...
app.get('/', (req, res) => {
    fs.readFile(path.resolve('../build/index.html'), 'utf8', async (err, data) => {
        ...
        const debates = await asyncQuery('SELECT * FROM debates;');
        let user = checkAuth(req);

        await Promise.all(debates.rows.map(async (debate, i) => {
            const commentsHere = await asyncQuery(`SELECT * FROM comments WHERE onPost = '${debate.id}';`);
...
const logIn = async (faceHash: string, password: string, res: any) => {
    ...
    if (existingUser.rows.length) {
        if (existingUser.rows[0].password === getHashedPassword(password)) {
...
app.post('/comment', async (req, res) => {
    ...
    if (!user) {
        ...
        await asyncQuery(`INSERT INTO users (id, isReply, text, onPost, fromUser) VALUES ('${id}', ${req.body.isReply}, '${rfc3986EncodeURIComponent(req.body.text)}', '${req.body.onPost}', '${user.username}');`);
...
app.post('/create', async (req, res) => {
    ...
    if (existingUser.rows.length) {
        ...
        await asyncQuery(`INSERT INTO users (id, username, passwordHash, faceHash) VALUES ('${uid}', '${req.body.username}', '${req.body.password}', '${req.body.faceHash}');`);
        const user = await logIn(req.body.faceHash, req.body.password, res);
        res.send(
            JSON.stringify(
                {
